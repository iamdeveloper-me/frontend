...
owner_id:string;
  ...
  constructor(public _service: CommonService,private not_service: NotificationsService,public http:Http) {
    this._service.getAllCompany().subscribe((data)=>{
      ...
      .pipe(
        startWith(''),
        map(statea => statea ? this._filterStatesA(statea) : this.statesA.slice())
      );
  ...
  onSubmit(){
    ...
    this.http.post("http://192.168.0.115:8080/api/job/",this.job).subscribe((data)=>{
      ...
      if(data.json().data.responseCode == '200'){
        ...
        this.not_service.error('Error !', data.json().data.error['job_name'], {
          timeOut: 3000,
          showProgressBar: true,
          pauseOnHover: true,
          clickToClose: true
